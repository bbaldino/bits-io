...
pub trait BitBufMutExts: BitBufMut {
    ...
    //     }
    //
    //     // TODO: BytesMut converts everything to a slice and then just does put_slice, so it'd be
    // nice     // to do the same.  The issue is that the uX types don't convert cleanly into
    // the BitSlice<u8,     // Msb0> semantics: a u1 is in a u8, but the subset of those bits
    // are in Msb0 order but shifted     // to the "right" side of the value's storage.  It
    // doesn't work to just treat it as Lsb0 order,     // because then the relevant bits would
    // be reversed: what we need to do is shift the bits to     // the left.  Or, we could slice
    // off the 'end' of the value where the bits are --> this works     //
    //     // But do we still have an issue for values larger than a byte?  does the above process
    // work     // with endian-ness?  Can we convert the inner-storage value to le/be bytes and
    // then bitslice     // it?
    //     //
    //     //
    //
    ...
    //         // TODO: this doesn't work, as this will left-pad a single bit value into 8 bits so
    // it'll         // get written as 0b00000001 instead of 0b1
