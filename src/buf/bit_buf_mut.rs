...
pub trait BitBufMut {
    ...
    /// Returns a mutable `UninitSlice` starting at the current `BitBufMut` position and of length between 0
    /// and BitBufMut::remaining_mut(). Note that this can be shorter than the whole remainder of
    /// the buffer (this allows non-continuous implementation).  This `BitBufMut` must be fully
    /// byte-aligned for this to work: caller should check `byte_aligned` before calling.
    ///
    /// This is a lower level function. Most operations are done with other functions.
    ///
    /// The returned byte slice may represent uninitialized memory and should not be read from.
    ...
    fn put_slice(&mut self, src: &BitSlice) {
        self.try_put_slice(src).unwrap()
    }
    ...
    fn try_put_slice(&mut self, mut src: &BitSlice) -> std::io::Result<()> {
